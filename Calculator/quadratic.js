...
function quadratic() {
  ...
  if (a === 0) {
    ...
    //a is not zero, so we really are considering a quadratic equation!
    ...
    if (
      ...
      Math.floor(Math.sqrt(Math.abs(discriminant)))
    ) {
      discrIsSquare = true;
    } else {
      discrIsSquare = false;
    ...
    if (discParity === 1) {
      ...
      if (discrIsSquare) {
        if (Math.sqrt(discriminant) % (2 * a) === 0 && b % (2 * a) === 0) {
          ...
          //simplify Numerator
          x1 =
            (-b + Math.sqrt(discriminant)).toString() +
            "/" +
            (2 * a).toString();
          x2 =
            (-b - Math.sqrt(discriminant)).toString() +
        ...
        //discrIsSquare === false
        if (isBZero) {
          //term only involves sqrt part of numberator and denominator.
          x1 = "sqrt(" + discriminant.toString() + ")/" + (2 * a).toString();
          x2 = "-sqrt(" + discriminant.toString() + ")/" + (2 * a).toString();
        } else {
          //discriminant is not a square and b is not zero, we don't simplify
          x1 =
            "(" +
            (-b).toString() +
            "+sqrt(" +
    ...
    } else if (discParity === -1) {
      //discriminant is negative
      if (discrIsSquare) {
        if (
          Math.sqrt(Math.abs(discriminant)) % (2 * a) === 0 &&
          b % (2 * a) === 0
        ) {
          x1 =
            ((-b + Math.sqrt(Math.abs(discriminant))) / (2 * a)).toString() +
            "i";
          x2 =
            ((-b - Math.sqrt(Math.abs(discriminant))) / (2 * a)).toString() +
            "i";
          ...
          //simplify Numerator
          x1 =
            (-b + Math.sqrt(Math.abs(discriminant))).toString() +
            "i/" +
            (2 * a).toString();
          x2 =
            (-b - Math.sqrt(Math.abs(discriminant))).toString() +
        ...
        if (isBZero) {
          ...
          x1 =
            "sqrt(" +
            Math.abs(discriminant).toString() +
            "i)/" +
            (2 * a).toString();
          x2 =
            "-sqrt(" +
      ...
      //discParity ===0 discriminant is 0, so considering -b/2a
      if (b % (2 * a) === 0) {
        x1 = (-b / (2 * a)).toString();
        x2 = (-b / (2 * a)).toString();
      } else {
        //no square root in answer and don't simplify
        x1 = (-b).toString() + "/" + (2 * a).toString();
        x2 = (-b).toString() + "/" + (2 * a).toString();
      }
    }
    if (x1 === x2) {
      //we have a double root because the discriminant was zero!
      quadraticAnswerDiv.innerHTML = `<h4> The solution is x = ${x1}. <br> 
            Answer is double root because the discriminant, -b-4*a*c, is 0.</h4>`;
    } else {
      //answer is not a double root.
      if (discParity === -1) {
        quadraticAnswerDiv.innerHTML = `<h4>The solutions are x = ${x1} and x = ${x2}. <br>
                Answer is imaginary because the discriminant, -b-4*a*c, is ${discriminant}.</h4>`;
      } else {
        //answer is real!!!
        quadraticAnswerDiv.innerHTML = `<h4>The solutions are x = ${x1} and x = ${x2}. <br>
                Answer is real because the discriminant, -b-4*a*c, is ${discriminant}.</h4>`;
      }
    }
  }
